/* https://magento.algolia.com/js/algoliasearch/internals/frontend/common.js */
var algolia = {
  allowedHooks: [
  ...
  triggerHooks: function () {
    ...
    var data = this.getRegisteredHooks(hookName).reduce(function (
      currentData,
      hook
    ) {
  ...
  algoliaBundle.$(function ($) {
    ...
    window.transformHit = function (hit, price_key, helper) {
      ...
      if (helper && algoliaConfig.useAdaptiveImage === true) {
        if (hit.images_data && helper.state.facetsRefinements.color) {
          matchedColors = helper.state.disjunctiveFacetsRefinements.color.slice(
            0
          );
        }
        if (
      ...
      if (section.name === 'products') {
        ...
        source = {
          source: $.fn.autocomplete.sources.hits(
            algolia_client.initIndex(
              algoliaConfig.indexName + '_' + section.name
            ),
            options
          ),
          name: section.name,
          templates: {
            empty: function (query) {
              var template =
                ...
                ' "' +
                $('<div>').text(query.query).html() +
                '"</div>';
              var suggestions = [];
              if (
                algoliaConfig.showSuggestionsOnNoResultsPage &&
                algoliaConfig.popularQueries.length > 0
              ) {
                $.each(
                  algoliaConfig.popularQueries.slice(
                    0,
                    Math.min(3, algoliaConfig.popularQueries.length)
                  ),
                  function (i, query) {
            ...
            suggestion: function (hit, payload) {
              ...
              return algoliaConfig.autocomplete.templates[section.name].render(
                hit
              );
            },
          },
          ...
          templates: {
            ...
            suggestion: function (hit, payload) {
              ...
              if (
                ...
                if (
                  hit.content.charAt(0).toUpperCase() !== hit.content.charAt(0)
                ) {
                  hit.content = '&#8230; ' + hit.content;
                }
                if (
                  $.inArray(hit.content.charAt(hit.content.length - 1), [
                    '.',
                    '!',
                    '?',
                  ])
        ...
        var suggestions_index = algolia_client.initIndex(
            algoliaConfig.indexName + '_suggestions'
          ),
          products_index = algolia_client.initIndex(
            algoliaConfig.indexName + '_products'
          ),
          suggestionsSource;
        if (algoliaConfig.autocomplete.displaySuggestionsCategories == true) {
          suggestionsSource = $.fn.autocomplete.sources.popularIn(
            suggestions_index,
            options,
            {
              source: 'query',
          ...
          suggestionsSource = $.fn.autocomplete.sources.hits(
            suggestions_index,
            { hitsPerPage: section.hitsPerPage }
          );
        }
            ...
            suggestion: function (hit, payload) {
              ...
              if (hit.facet && hit.facet.value !== 'All departments') {
                hit.url =
                  ...
                  '#q=' +
                  hit.query +
                  '&hFR[categories.level0][0]=' +
                  encodeURIComponent(hit.category) +
                  '&idx=' +
              ...
              hit._highlightResult.query.value = algoliaBundle.autocomplete.escapeHighlightedString(
                toEscape
              );
              hit.__queryID = payload.queryID;
              hit.__position = payload.hits.indexOf(hit) + 1;
        ...
        var index = algolia_client.initIndex(
          algoliaConfig.indexName + '_section_' + section.name
        );
        source = {
          ...
          templates: {
            suggestion: function (hit, payload) {
              hit.url =
                algoliaConfig.baseUrl +
                '/catalogsearch/result/?q=' +
                encodeURIComponent(hit.value) +
                '&refinement_key=' +
                section.name;
              hit.__queryID = payload.queryID;
              hit.__position = payload.hits.indexOf(hit) + 1;
              return algoliaConfig.autocomplete.templates.additionnalSection.render(
                hit
              );
            },
          },
      ...
      if (section.name === 'products') {
        source.templates.footer = function (query, content) {
          ...
          var returnFooter =
            ...
            allUrl +
            '">' +
            algoliaConfig.translations.allDepartments +
            '</a></span> (' +
            content.nbHits +
            ')';
    ...
    window.fixAutocompleteCssHeight = function () {
      if ($(document).width() > 768) {
        ...
        var height = Math.max(
          $otherSections.outerHeight(),
          $dataSetProducts.outerHeight()
        );
    ...
    window.fixAutocompleteCssSticky = function (menu) {
      ...
      if (
        menu.offset().left + menu.outerWidth() / 2 >
        $(document).width() / 2
      ) {
        ...
        currentOffset = Math.round(
          autocomplete_container.offset().left +
            autocomplete_container.outerWidth()
        );
    ...
    $(window.algoliaConfig.autocomplete.selector).each(function () {
      $(this)
        ...
        .submit(function (e) {
          ...
          if (algoliaConfig.instant.enabled && query == '') query = '__empty__';
    ...
    function handleInputCrossAutocomplete(input) {
      if (input.val().length > 0) {
        input
          .closest('#algolia-searchbox')
          .find('.clear-query-autocomplete')
          .show();
    ...
    window.handleInputCrossInstant = function (input) {
      if (input.val().length > 0) {
        input
          .closest('#instant-search-box')
          .find('.clear-query-instant')
          .show();
      } else {
        input
          .closest('#instant-search-box')
    ...
    var instant_selector = !algoliaConfig.autocomplete.enabled
    ...
    $('body').on('click', '#refine-toggle', function () {
      ...
      else $(this).html('+ ' + algoliaConfig.translations.refine);
    });
