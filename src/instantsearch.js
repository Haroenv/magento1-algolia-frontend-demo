/* https://magento.algolia.com/js/algoliasearch/instantsearch.js */
document.addEventListener('DOMContentLoaded', function (event) {
  algoliaBundle.$(function ($) {
    ...
    search.client.addAlgoliaAgent(
      'Magento integration (' + algoliaConfig.extensionVersion + ')'
    );
    ...
    var allWidgetConfiguration = {
      hits: {
        ...
        templates: { allItems: $('#instant-hit-template').html() },
        transformData: {
          allItems: function (results) {
            for (var i = 0; i < results.hits.length; i++) {
              results.hits[i] = transformHit(
                results.hits[i],
                algoliaConfig.priceKey,
                search.helper
              );
              results.hits[i].isAddToCartEnabled =
                algoliaConfig.instant.isAddToCartEnabled;
      ...
      stats: {
        ...
        templates: { body: $('#instant-stats-template').html() },
      ...
      currentRefinedValues: {
        ...
        cssClasses: { root: 'facet' },
        templates: {
          header:
            '<div class="name">' +
            algoliaConfig.translations.selectedFilters +
            '</div>',
        ...
        transformData: {
          allItems: function (results) {
            ...
            for (var i = 0; i < results.hits.length; i++) {
              ...
              results.hits[i].isAddToCartEnabled =
                algoliaConfig.instant.isAddToCartEnabled;
              results.hits[i].__position =
                state.page * state.hitsPerPage + ++hitIndex;
              results.hits[i].algoliaConfig = window.algoliaConfig;
            }
            return results;
          },
    ...
    var customAttributeFacet = {
      categories: function (facet, templates) {
        ...
        var hierarchicalMenuParams = {
          ...
          templates: templates,
          sortBy: ['name:asc'],
          cssClasses: { list: 'hierarchical', root: 'facet hierarchical' },
        };
    ...
    window.getFacetWidget = function (facet, templates) {
      ...
      if (facet.type === 'disjunctive') {
        var refinementListOptions = {
          ...
          operator: 'or',
          templates: templates,
          cssClasses: { root: 'facet disjunctive' },
        };
        refinementListOptions = addSearchForFacetValues(
          facet,
          refinementListOptions
    ...
    function startInstantSearch() {
      ...
      $(document).on(
        'click',
        '.ais-hierarchical-menu--link, .ais-refinement-list--checkbox',
        function () {
          focusInstantSearchBar(search, instant_search_bar);
        }
      );
